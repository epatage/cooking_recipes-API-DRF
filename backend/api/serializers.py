...
class IngredientPostSerializer(serializers.ModelSerializer):
    """Сериалайзер для передачи ингредиента при публикации рецепта."""
    ...
    def to_internal_value(self, data):
        """Для создания/редактирования рецептов по id ингредиентов."""
        return data
...
class RecipeGetAuthorizedSerializer(RecipeGetSerializer):
    ...
    def __get_field_method(self, obj, model):
        return model.objects.filter(
            user=self.context["request"].user, recipe=obj
        ).exists()

    def get_is_favorited(self, obj):
        return self.__get_field_method(obj=obj, model=Favorite)

    def get_is_in_shopping_cart(self, obj):
        return self.__get_field_method(obj=obj, model=ShoppingCart)
